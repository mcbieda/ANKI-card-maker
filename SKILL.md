...
5. Note that if chatgpt web addresses are given, these will be dialogues with the LLM.
	...
	- the dialogue should be carefully read under these conditions and the user's weaknesses evaluated.
	- cards should especially emphasize areas that the user had trouble with
	- claude.ai/share links (and other LLM chat UIs) don't have an extractor script — they are pure client-rendered pages with no conversation content in the page source at all, so there is nothing to fetch or decode. For these, the user will instead hand you a plain text file containing a copy-pasted transcript (often named with a `chat-`/`dialogue-` prefix as a hint, but don't rely on the filename alone — go by what the user tells you about it).
